...
if ($isStaff && (int)$module['ModuleLeaderID'] !== $currentStaffId) {
    header("Location: ../view/modules.php?error=unauthorized");
    exit;
}

$staff = $isStaff ? [] : $pdo->query("SELECT * FROM Staff ORDER BY Name")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'update_module') {
        ...
        $leader_id = $isStaff ? $currentStaffId : (int)($_POST['leader_id'] ?? 0);

        if ($name === '' || $description === '' || $leader_id <= 0) {
            $error = "Please fill in all module fields.";
        } else {
            if ($isStaff) {
                $stmt = $pdo->prepare("UPDATE Modules SET ModuleName = ?, Description = ? WHERE ModuleID = ? AND ModuleLeaderID = ?");
                $updated = $stmt->execute([$name, $description, $id, $currentStaffId]);
            } else {
                $stmt = $pdo->prepare("UPDATE Modules SET ModuleName = ?, Description = ?, ModuleLeaderID = ? WHERE ModuleID = ?");
                $updated = $stmt->execute([$name, $description, $leader_id, $id]);
            }
...
<title>Edit Module | Student Course Hub</title>
    ...
    <main class="admin-content">
        <div class="admin-header">
            ...
            <div class="admin-actions">
                <a href="../view/module-view.php?id=<?= $id ?>" class="admin-btn admin-btn-view">
                    <span class="nav-icon"></span> View
                </a>
                <a href="../view/modules.php" class="admin-btn admin-btn-view">
                    ...
                    <div class="admin-card-body">
                        <form method="POST" class="admin-form" style="padding: 0;">
                            ...
                            <div class="admin-form-group">
                                ...
                                <input type="text" id="name" name="name" value="<?= htmlspecialchars($module['ModuleName']) ?>" placeholder="Enter module name" required>
                            </div>
                            ...
                            <?php if ($isStaff): ?>
                                <input type="hidden" name="leader_id" value="<?= (int)$currentStaffId ?>">
                                <div class="admin-form-group">
                                    <label>Module Leader</label>
                                    <input type="text" value="Your account" placeholder="Current account" disabled>
                                </div>
                            <?php else: ?>
                                <div class="admin-form-group">

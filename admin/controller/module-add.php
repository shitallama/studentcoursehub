...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($leader_id <= 0) {
        $error = 'Unable to determine module leader for this account.';
    } else {
        ...
        try {
            ...
            $pdo->commit();
            header("Location: ../view/modules.php?msg=added");
            exit;
        } catch (Exception $e) {
            $pdo->rollBack();
...
<title>Add Module | Student Course Hub</title>
                ...
                <form method="POST" class="admin-form">
                    ...
                    <div class="admin-form-actions">
                        <button type="submit" class="admin-btn admin-btn-primary">
                            <span class="nav-icon"><span class="icon-svg icon-add" aria-hidden="true"></span></span> <?= $copy_data ? 'Duplicate Module' : 'Create Module' ?>
                        </button>
                        <a href="../view/modules.php" class="admin-btn admin-btn-view">Cancel</a>
                    </div>
                </form>
            </div>
        </div>

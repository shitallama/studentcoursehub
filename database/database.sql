...
DROP TABLE IF EXISTS AuthRateLimits;
...
CREATE TABLE AuditLogs (
    ...
    Details TEXT NULL,
    CreatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_auditlogs_actiontype (ActionType),
    INDEX idx_auditlogs_createdat (CreatedAt),
    INDEX idx_auditlogs_targetuserid (TargetUserID),
    INDEX idx_auditlogs_targetstaffid (TargetStaffID),
    ...
    FOREIGN KEY (TargetStaffID) REFERENCES Staff(StaffID) ON DELETE SET NULL
);
...
-- One-time migration for existing environments that predate FullName.
-- Safe to keep in source control; run once during upgrade.
ALTER TABLE Users ADD COLUMN IF NOT EXISTS FullName VARCHAR(120) NULL AFTER Username;
UPDATE Users
...
VALUES ('SuperAdmin', 'admin@edu.nielsbrock.dk', '$2y$10$zrg0A5lYwmKSjftbjw/K.eF645sADFzFj67XyxQhd0utTCUCXPBtW', 'admin');